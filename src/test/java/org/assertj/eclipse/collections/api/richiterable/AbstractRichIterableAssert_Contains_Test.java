...
class AbstractRichIterableAssert_Contains_Test {
  @RichIterableParameterizedTest
  void passesSingleValue(RichIterableAssertFactory<String> assertFactory) {
    assertThatNoException().isThrownBy(() ->
      assertFactory.fromElements("TOS", "TNG", "DS9", "VOY", "ENT").contains("DS9"));
  }

  @RichIterableParameterizedTest
  void passesMultipleValues(RichIterableAssertFactory<String> assertFactory) {
    assertThatNoException().isThrownBy(() ->
      assertFactory.fromElements("TOS", "TNG", "DS9", "VOY", "ENT").contains("TOS", "VOY", "ENT"));
  }

  @RichIterableParameterizedTest
  void passesBothEmpty(RichIterableAssertFactory<String> assertFactory) {
    assertThatNoException().isThrownBy(() ->
      assertFactory.fromEmpty().contains());
  }

  @RichIterableParameterizedTest
  void fails(RichIterableAssertFactory<String> assertFactory) {
    assertThatExceptionOfType(AssertionError.class)
      .isThrownBy(() -> assertFactory.fromElements("TOS", "TNG", "DS9", "VOY", "ENT").contains("DIS"))
  ...
  void failsWithMultipleValues(RichIterableAssertFactory<String> assertFactory) {
    assertThatExceptionOfType(AssertionError.class)
      .isThrownBy(() -> assertFactory.fromElements("TOS", "TNG", "DS9", "VOY", "ENT").contains("DS9", "DIS"))
      .withMessageContaining("to contain")
      .withMessageContaining("but could not find the following element(s)")
      .withMessageContaining("DIS");
  }
  ...
  void failsEmpty(RichIterableAssertFactory<String> assertFactory) {
    assertThatExceptionOfType(AssertionError.class)
      .isThrownBy(() -> assertFactory.fromEmpty().contains("DS9"))
      .withMessageContaining("to contain")
      .withMessageContaining("but could not find the following element(s)")
  ...
  void failsNull(RichIterableAssertFactory<String> assertFactory) {
    assertThatExceptionOfType(AssertionError.class)
      .isThrownBy(() -> assertFactory.fromNull().contains("DS9"))
  ...
  void failsNullInput(RichIterableAssertFactory<String> assertFactory) {
    assertThatExceptionOfType(NullPointerException.class)
      .isThrownBy(() -> assertFactory.fromElements("TOS", "TNG", "DS9", "VOY", "ENT").contains(null))
  ...
  void softAssertionPasses(RichIterableAssertFactory<String> assertFactory) {
    SoftAssertions.assertSoftly(softly -> assertFactory.softlyFromElements(softly, "TOS", "TNG", "DS9", "VOY", "ENT").contains("DS9"));
  }
}
